...
# "Keine": die Funktion bekommt keine Taste, SC Info faengt dann auch nichts
# ab - das Spiel behaelt alle F-Tasten (Wunsch aus der Gilde).
KEINE = "Keine"


def taste_vk(name, tabelle, standard):
    """Tastenname -> virtueller Tastencode; "Keine" -> 0 (wird nie ausgeloest)."""
    if name == KEINE:
        return 0
    return tabelle.get(name, standard)
...
class TastenWaechter(QThread):
    ...
    def __init__(self, vk_messen, vk_ablauf, vk_aus, vk_fenster=0x4D):
        ...
        self.zuordnung = {vk: k for vk, k in ((vk_messen, 1), (vk_aus, 3), (vk_ablauf, 2))
                          if vk}                # 0 = "Keine" -> nicht abfangen
        self._laufen = True
        self._letzte = 0.0
        ...
        def rueckruf(code, wparam, lparam):
            ...
            if code == 0:
                try:
                    ...
                    if self.vk_fenster and vk == self.vk_fenster and (daten.flags & 0x20):
...
class Fenster(QMainWindow):
    ...
    def _kopf(self):
        ...
        def auswahl(text, eintraege, aktuell, breite, slot, tipp=""):
            ...
            box = QComboBox()
            box.addItems([KEINE] + list(eintraege))
        ...
        for box in (self.auto_wahl, self.taste_wahl, self.aus_wahl, self.fenster_wahl):
            box.setItemData(0, "„Keine“: diese Funktion bekommt keine Taste – "
                               "das Spiel behält sie.", Qt.ItemDataRole.ToolTipRole)
    ...
    def _status_setzen(self):
        ...
        teile = [f"{t}: {was}" for t, was in ((auto, "Anzeige an + messen"),
                                              (mess, "nur messen"), (aus, "Anzeige aus"),
                                              (kombi, "Fenster vor/zurück")) if t != KEINE]
        if not teile:
            teile.append("keine Tasten belegt – Bedienung per Maus")
    ...
    def _waechter_starten(self):
        ...
        vk_mess = taste_vk(self.einst.get("taste", "F9"), VK_TASTEN, 0x78)
        vk_auto = taste_vk(self.einst.get("taste_ablauf", "F6"), VK_TASTEN, 0x75)
        vk_aus = taste_vk(self.einst.get("taste_aus", "F7"), VK_TASTEN, 0x76)
        vk_fenster = taste_vk(self.einst.get("fenster_kombi", "Alt+M"), FENSTER_KOMBIS, 0x4D)
    ...
    def hilfe_zeigen(self):
        ...
        an, mess, aus, kombi = ("– (keine Taste)" if t == KEINE else t
                                for t in (an, mess, aus, kombi))
        html = f"""
        <style>
        ...
        <p class="dim">Die Spielkonsole öffnet SC Info mit <b>^</b> (links neben der 1) – das ist in Star Citizen fest. Alle anderen Tasten sind oben im Fenster umstellbar. Die gewählten Tasten gehören SC Info allein – das Spiel bekommt sie nicht mehr. Wer eine Funktion nicht braucht, wählt <b>„Keine“</b> – dann bleibt die Taste beim Spiel; die Knöpfe im Fenster gehen weiterhin.</p>
        ...
        <p>Daraus entsteht die Note von 0 bis 10 mit klarer Empfehlung: <b>bleiben</b> oder <b>neu joinen</b>. Gewichtung: Server-FPS 40 %, Ping 25 %, Hitches 20 %, Paketverlust 10 %, Shard-Auslastung 10 %. Ein Server mit 30 FPS, 40 ms Ping und 0 Hitches landet bei rund 9,5 – Ruckeln trotz guter Note kommt dann vom eigenen Rechner (Grafiklast, Client-FPS), nicht vom Server. Die Ampel je Zeile: grün = gut, gelb = mittel, rot = schlecht, grau = nicht gelesen, <b>blau = nur Information</b> (Region, Standort, Bandbreite – wird nicht bewertet).</p>
